...
export async function createDestination(
  ...
  try {
    const body = req.body;
    const checkBanner = await Destination.findOne({
      title: body.title,
    });
    if (checkBanner) {
      throw new HttpException(
    ...
    const banner = await Destination.create({
      ...req.body,
    });
...
export async function updateDestination(
  ...
  try {
    ...
    const banner = await Destination.findOne({
      _id: _id,
    });
    ...
    const _banner = await Destination.findByIdAndUpdate(
        _id,
